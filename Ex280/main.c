...
//Exercicio 270: Busca em Árvore Binária de Busca - 2ª VERSÃO
...
{
    while(binaryTree)
    /*Enquanto o ponteiro analisado possuir um endereço de memória*/
    {
        if(value == binaryTree->value) return binaryTree;//Caso o valor seja encontrado na árvore binária
        else {
            if(value < binaryTree->value) binaryTree = binaryTree->left;//Caso o valor seja menor que o valor do nó analisado, o ponteiro irá receber o nó a esquerda
            else binaryTree = binaryTree->right;//Caso o valor seja maior que o valor do nó analisado, o ponteiro irá receber o nó a direita
        }
    }
    return NULL;//Caso o ponteiro possua um valor nulo, ou seja, caso o valor não seja encontrado.
}
